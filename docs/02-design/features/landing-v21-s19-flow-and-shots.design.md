...
## 2. T3 흐름도 — **구현했다가 철회** 〔2026-08-24〕

> 🔴 **랜딩에서 뺐습니다** 〔2026-08-24 · 사용자 판단〕. 같은 날 구현·배포(`b845ba6`)
> 했다가 되돌렸습니다. 흐름도 섹션·CSS·전용 테스트(`landing-flowmap-s19`)를 모두
> 걷었고, 그때 밀어 둔 하류 8개 섹션의 배경도 원래대로 되돌렸습니다.
> 함께 들어갔던 두 가지는 **남겼습니다** — 흐름도와 무관하기 때문입니다:
> §5-17 상품 ② 캡션 교체(캡처가 홈 대시보드로 바뀐 사실은 그대로), 그리고
> `en.html` 의 `.pricing-inner` 세로 여백 결손 보완(선재 결함).
> 자산 두 개(`flow-how.svg` · `flow-how.jpg`)는 git 추적 상태로 저장소에 남깁니다.
>
> 아래 실측은 **다시 넣을 때를 위해 남겨 둡니다.**
