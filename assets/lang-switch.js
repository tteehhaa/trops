 ...
 * 〔2026-08-30 갱신 · 대표 지시로 기본이 국문으로 되돌아감 — 아래 배너 로직 자체는
 *   그대로 유효하다. `lang=en` 쿠키를 직접 골랐거나 /en* 을 직접 열어서 온 경우에만
 *   여전히 도달하는 화면이 영문이기 때문이다〕
 *
 * 기본은 국문(middleware.js 가 `lang=en` 쿠키가 있을 때만 국문 경로를 /en* 로
 * 리다이렉트한다). 이 파일이 하는 일은 둘뿐이다 — ① 영문 페이지에서, 브라우저 언어가
 ...
 *
 * 🔴 **이 파일이** 쓰는 저장소는 쿠키 하나(`lang` = ko|en)뿐이다 — 광고·분석 목적이
 *    아니고, 다른 방문과 연결해 이용자를 식별하지 않는다(privacy.html §01 첫째 예외,
 *    2026-08-21). track.js 와 같은 이유로 그 이상은 만들지도 읽지도 않는다.
 *    ⚠️ 사이트 전체가 쿠키 하나라는 뜻이 아니다 〔2026-09-12〕 — 8장 <head> 의 GA4
 *       스니펫이 `_ga` 계열을 심고, 방침 §01 의 셋째 예외가 그것을 적는다.
 *
