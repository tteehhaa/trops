...
/*
 * 판매가 — **실제 결제 금액, VAT 포함 총액**. 서버가 가진 이 값만 신뢰합니다(클라이언트
 * 전달값 신뢰 금지).
 *
 ...
 *
 * 🔴 **₩300,000 → ₩330,000** 〔2026-08-17 · 대표 결정 · VAT 반영〕. 대표가 법인이고
 *    법인은 간이과세 대상이 아니라 매출 규모와 무관하게 일반과세자입니다(부가가치세법
 *    §61). 종전 「₩300,000(VAT별도)」 표기는 공급가액을 뜻했으나 이 상수는 그 부가세를
 *    더하지 않은 채 청구하고 있었습니다 — 표시와 실제 청구가 갈려 있던 상태입니다.
 *    이제 이 값이 **공급가액 300,000 + 부가세 30,000 = 330,000** 실제 청구·표시 총액
 *    입니다. 화면 표기도 공급가·세액 분리가 아니라 총액 하나 + 「VAT 포함」 병기로
 *    맞춥니다.
 *
