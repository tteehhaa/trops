 ...
 * 정가.
 *
 * ⚠️ 2026-08-11 부터 **화면에 그리지 않습니다** 〔R-1〕. 종전에는 precheck.html 의
 *    플랜 카드·결제 요약에 취소선으로 붙었는데, ₩290,000 은 실제 판매 이력이 없어
 *    「종전거래가격」 표시 요건을 채우지 못합니다. 취소선을 뗀 이유가 마크업이
 *    아니라 표시 자체이므로 다시 그리려면 그것이 결정 사안입니다.
 *
 * ⛔ 값은 지우지 않습니다. trops_a 의 정본(lib/payment/precheck-paid-gate.ts
 *    PRECHECK_PRICE.listKrw)과 대조되는 사본이라, 여기서 지우면 대장이 가리키는
 *    좌표가 사라지고 드리프트 검출이 조용히 죽습니다.
 *
 * 🔴 2026-08-13 부터 이 값은 **판매가보다 낮습니다**(290,000 < 300,000). 「정가」로 쓸 수
 *    없는 상태이며, 게시하면 앵커가 아니라 **역앵커**가 됩니다. R-1 이 이미 표기를 전
 *    페이지에서 걷어냈고 verify-deployment.js 「R1-비교표기」가 0건을 단정합니다.
 *    ⛔ 이 값을 화면으로 되돌리지 마십시오.
 *
 * 지금 쓰이는 곳: api/payment-config.js 가 listPrice 로 내려보냅니다(화면 미사용).
 */
