 ...
 *
 * ── 🔴 트리거가 둘입니다 〔M-2 재착수로 하나가 늘었습니다 · 2026-08-11〕 ────────
 *
 *   사람   scripts/refund.js <orderId> --reason "…"
 *          사유를 사람이 적습니다. 「판별이 뒤집혔다」를 사람이 확인한 경우,
 *          그리고 **자료를 이미 전달한 뒤** 환불하는 경우(환불규정 §03 판단)입니다.
 *
 *   판정층 api/cron/refund-blocked.js → api/_route-refund.js → 이 파일
 *          판정층 trops_a 가 precheck_intake_route 에 route='blocked' 를 적으면
 *          하루 한 번 그것을 보고 환불합니다. 사유는 표에서 옵니다.
 *
 * ⚠️ 전에는 이 자리에 「자동이 불가능하다」가 적혀 있었습니다 — 처리 가능 여부를
 *    받아오는 인터페이스가 없었기 때문입니다(M-2 스코프 아웃의 원인 ·
 *    docs/verify/2026-08-11-batch-c2.md). 그 인터페이스가 서면서 조건이 바뀌었습니다.
 *    바뀌지 않은 것은 **이 파일에 판단이 없다**는 사실입니다 — 사유는 언제나
 *    인자로 들어옵니다. 이 저장소가 「불가인지」를 스스로 정하기 시작하면 그것이
 *    곧 이중구현이고 경계 위반입니다.
 *
 ...
 *
 *   판정층 판단으로 도는 쪽은 scripts/refund-blocked.js(미리보기·수동) ·
 *   api/cron/refund-blocked.js(정기)이고, 둘 다 **이 함수를 그대로** 부릅니다.
 *   취소 순서를 그쪽에 다시 쓰지 않았습니다 — 복제하면 한쪽만 고쳐집니다.
 *
 * ── 순서가 설계다 ───────────────────────────────────────────────────────────
