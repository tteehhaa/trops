...
/* ── A3 §6 · 법률자문 오인 재프레이밍 ────────────────────────────────────────
 *
 * 2026-08-16 대표 확정. 아래 두 검사는 **문면을 하드코딩하지 않습니다** — 두 자리에
 * 같은 문장이 있는지, 그 문장이 금지 규칙을 지키는지만 봅니다. 문구가 나중에 다시
 * 바뀌어도 이 파일은 고칠 것이 없고, 한쪽만 고치면 그때 걸립니다. 그것이 이 검사의
 * 존재 이유입니다.
 * ⛔ 두 줄을 **걷어내기로** 결정되면, 그때는 아래 두 test 도 함께 지우십시오 —
 *    문장이 사라지면 이 검사는 뜻 없이 실패합니다.
 *
