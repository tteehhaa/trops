 ...
 *   S4 FAQ 채널분기 문항 — 🔴 2026-08-16 A3 §4 로 삭제됨. 검사도 함께 뺐습니다
 *      (파일 하단 「── S4 」 절 참조).
 *
...
/*
 * 🔴 **채널분기 문항 자체가 삭제됐습니다** 〔2026-08-16 · docs/a3/trops_A3_최종구조_정리.md
 *    §4 삭제 대상 3〕. 신설 사유(흐름 md §4·§5-1 13번 — CTA 혼동 보완)는 소멸하지
 *    않았지만, 그 답이 이제 **화면 구조 자체**가 지고 있습니다: 히어로·상품 탭·결
 *    CTA(→ /precheck)와 히어로·마감 CTA(→ #interest, 「문의하기」)가 이미 다른
 *    버튼·다른 목적지로 갈려 있어, 「무엇이 다른가」를 글로 다시 설명할 필요가
 *    없다는 것이 A3 §4 의 판단입니다.
 * ⛔ 아래 두 검사(문항 존재 · 묶음 위치)를 되살리지 마십시오. 되살리려면 그
 *    항목부터 index.html 에 다시 넣어야 하고, A3 §4 의 판단을 뒤집는 일입니다.
 */
