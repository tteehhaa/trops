...
test('🔴 폐기된 상품명이 어느 페이지 본문에도 없다', () => {
   ...
   */
  /*
   * 🔴 **폐기된 «산출물» 명칭을 더했습니다** 〔2026-09-03 · 대표 지시〕. PRD §7 · §8-1
   *    결정 1 이 산출물 명칭을 `사전 점검 리포트` 로 정하고 그 낱말을 폐기했는데,
   *    그것을 재던 `scripts/check-b3a-gates.js` G3 가 **6주간 red 였습니다**(뒤 배치가
   *    대상 페이지를 교체해서). 그 사이 랜딩·precheck 에 **7곳**이 들어왔고 아무도
   *    못 봤습니다. 오늘 그 7곳을 활동 축으로 옮겼고, 감시를 여기로 되살립니다.
   * 🔴 **`body()` 만으로는 못 잡습니다** — 7곳 중 여럿이 `<script>` 안의 JS 문자열이었고
   *    `body()` 는 `<script>` 를 걷습니다. 아래가 `strip` 층까지 보는 것이 그래서 중요합니다.
   * ⚠️ refund·en-refund 는 이 낱말을 **HTML 주석**으로 들고 있습니다(이름 충돌의 역사).
   *    두 층 모두 HTML 주석을 걷으므로 걸리지 않습니다 — 그 기록은 남겨 둡니다.
   * ⛔ 위 규약대로 이 낱말도 **주석에 리터럴로 적지 마십시오**(JS 주석은 안 걷힙니다).
   */
  const RETIRED = ['NDA 비교', '문서 대조', '거래 절차 트래킹', '무역보험 서류 패키지', '무역보험 준비팩', '진단' + '서'];
  const offenders = [];
