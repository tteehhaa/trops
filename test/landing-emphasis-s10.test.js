...
test('E1 스토리 문구에 평가어가 없다', () => {
  ...
  /* 🔄 '문제' 를 뺐습니다 〔2026-08-23 · B3-a〕. §5-15 가 「문제는 이 정보들이 …
     흩어져 있어 …」로 확정됐고, 이것은 **읽는 사람을 판정하는 말이 아니라 상황을
     가리키는 말**입니다. 이 규칙이 막으려던 것(우리가 먼저 위험·불리·잘못을 선언하는 것)은
     그대로 지켜집니다. ⛔ 나머지 세 낱말은 되살리지 마십시오. */
  for (const word of ['위험', '불리', '잘못']) {
...
test('E5 근거 디테일이 화면에 그대로 있다 — 접지 않았다', () => {
  ...
  /* 🔄 2026-08-23 〔PRD v2.1 B3-a〕 §5-12·5-13·5-14·5-15·5-16 이식 반영. */
  const MUST = [
  ...
  for (const s of MUST) {
    assert.ok(BT.indexOf(s) !== -1, '근거 디테일이 사라졌습니다: 「' + s + '」');
  }
...
test('E10-a 캡처가 「정식 화면」이 아니라 「참고 이미지」로 보인다', () => {
  ...
  /* 🔄 2 → 3 〔2026-08-23 · B3-a 작업 6〕. 탭② 자리가 플레이스홀더로 채워지면서
     세 탭이 다시 각각 그림과 캡션을 갖습니다. */
...
test('E12 강약을 고치면서 콘텐츠·기능이 빠지지 않았다', () => {
  ...
  //    빠졌습니다. 로드맵 한 곳만 남습니다.
  /* 🔄 1 → 0 〔2026-08-23 · B3-a · §5-14〕. 로드맵의 미리보기 훅·버튼이 §5-14 에 없어
     빠졌습니다. 패널은 탭으로 그대로 열리고, JS 는 `if (!openers.length) return` 로
     막혀 있습니다. 사유는 test/landing-order-s9.test.js 의 같은 자리 주석 참조. */
  assert.strictEqual((B.match(/data-timeline-open/g) || []).length, 0, '기한관리 트리거가 남아 있습니다 — §5-14 에는 없습니다');
