...
/* ══ 1. 값 ═══════════════════════════════════════════════════════════════════
 *
 * 🔴 **이 절을 걷었습니다** 〔2026-09-09 · 앞단 Supabase 폐기〕.
 *    두 검사가 있었습니다 — `api/_payment.js` 의 `PRICE === 330000`, 그리고
 *    `api/payment-config.js` 가 그 값을 내려주는지. 둘 다 **지킬 근거가 사라졌습니다.**
 *
 *    그 자리 주석은 「상품은 더는 팔지 않지만 이 값은 **남은 접수 32건의 환불 근거**다 ·
 *    `scripts/refund.js` 가 이 금액으로 취소를 건다」였습니다. 그런데 2026-09-09 에
 *    접수 32건을 전량 삭제하고 앞단 프로젝트를 폐기했으며, `scripts/refund.js` 와
 *    결제 코드 일체를 함께 걷었습니다. **환불을 걸 대상도, 걸 도구도 없습니다.**
 *
 * ⛔ 금액 상수를 이 저장소에 다시 세우지 마십시오 — 결제는 app.trops.kr 소관입니다.
 * ⚠️ 아래 「노출」 절은 **그대로 살아 있습니다.** 파는 상품이 없으니 어느 페이지에도
 *    금액이 보이면 안 된다는 규칙이고, 그건 오히려 지금 더 중요합니다.
 */
