...
test('⚠️ #demo 앵커를 지우지 않았다 — 바깥에서 그리로 오는 링크가 있을 수 있다', () => {
  /*
   * 🔄 **축이 좁아졌습니다** 〔2026-09-05 · 대표 결정〕 — 「같은 문서 안의 링크」를 뺐습니다.
   *
   * 1단계 카드 설명문이 「…아래에서 «결과 예시»를 보실 수 있습니다」였고, 그 «결과 예시»가
   * 문서 안에서 `#demo` 로 가는 **유일한** 링크였습니다. 문구를 「이번 수출에서 우선 확인할
   * 부분을 점검합니다.」로 바꾸면서 그 링크가 함께 없어졌습니다 — 데모 섹션이 바로 아래에
   * 있어 스크롤로 닿는다는 판단입니다.
   *
   * ⛔ **`id="demo"` 는 그대로 지킵니다.** 문서 안에서 가는 길이 없어졌을 뿐, 바깥에서
   *    `trops.kr/#demo` 로 오는 링크(메일·광고·공유된 주소)는 여전히 그 id 를 찾습니다.
   *    id 를 지우면 그 링크들이 조용히 페이지 맨 위로 떨어집니다.
   * ⚠️ 문서 안에 링크를 다시 넣거든, 아래 단정도 함께 되살리십시오.
   */
  const html = read(LANDING);
