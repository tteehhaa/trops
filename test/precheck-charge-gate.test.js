...
test('payment-config 는 게이트 상태를 알린다 — 화면이 잠글 근거', async () => {
  await withFakes(async () => {
     ...
     *    ① 판매 이력이 없어 표시할 수 없고(R-1) ② 판매가(300,000)보다 낮아져 역앵커가
     *    됐습니다. **상수는 사본 좌표로 남기고 브라우저로 가는 길만 끊었습니다** —
     *    응답에 실려 있으면 「이미 내려오는 값이니 그려도 되겠지」로 읽힙니다.
     * ⛔ 이 단언을 되돌리지 마십시오. 되살리려면 게시 자체가 결정 사안입니다.
     */
    assert.ok(!('listPrice' in res.body),
      'listPrice 가 응답에 되살아났습니다 — 폐기된 정가가 브라우저까지 내려갑니다');
    // 값 자체는 살아 있어야 합니다(대장이 가리키는 사본 좌표).
    const payment = require('../api/_payment.js');
    assert.strictEqual(payment.LIST_PRICE, 290000,
      'LIST_PRICE 상수가 사라졌습니다 — 옆 저장소 드리프트 검출이 조용히 죽습니다');
  });
});
