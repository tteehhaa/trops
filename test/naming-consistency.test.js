 ...
 */
/*
 * 🔴 **흐름도 SVG 안의 한국어는 이 검사에서 뺍니다** 〔2026-08-24 · PRD §5-19〕.
 *
 * §5-19 는 도입부와 alt 의 **영문만** 확정했고, 도형 안 일곱 낱말(계약서·증권·품목·
 * TROPS·기한·요건·근거)의 영문 정본은 아직 없습니다. 정본 없이 옮겨 적으면 이 저장소가
 * PRD 를 문면 단일 정본으로 삼은 규율이 그 자리에서 깨집니다. 그래서 §5-19 가 지시한 대로
 * **SVG 원본을 그대로** 넣고, 이 예외를 여기 적어 둡니다.
 *
 * 화면을 읽지 못하는 사용자에게는 영향이 없습니다 — `<svg role="img" aria-label="…">` 가
 * §5-19 영문 alt 원문을 들고 있어 스크린리더는 영어로 읽습니다.
 *
 * ⚠️ **이월**: SVG 일곱 낱말의 영문 정본이 PRD 에 생기면 영문 전용 SVG 를 만들고
 *    이 예외를 지우십시오. 예외를 넓히지 마십시오 — `.flowmap-fig` 하나만 빼고 있고,
 *    다른 곳의 미번역 한글은 여전히 이 검사가 잡습니다.
 */
test('영문 페이지의 화면 문구에 한글이 남아 있지 않다', () => {
  for (const en of EN_PAGES) {
    const text = body(read(en))
      .replace(/<figure class="flowmap-fig">[\s\S]*?<\/figure>/g, '')
 ...
 *
 * `assets/auth.js` 는 비로그인 헤더의 [로그인] 버튼을 `class="nav-quiet ta-link"` 로
 * 그립니다 — 색과 글자 크기 사본을 auth.css 에 두지 않아서 같은 줄 왼쪽의 [EN]·[한국어] 와
 * 회색이 갈리지 않습니다. 대신 **의존이 파일을 건넙니다**:
 * 규칙은 각 페이지의 <style> 안에 있고, 쓰는 곳은 별 파일(auth.js)입니다.
 *
 * 이 검사가 막는 것은 **조용한 실패**입니다. `.nav-quiet` 를 지우면 그 버튼이 색·크기를
 * 잃고 브라우저 기본 버튼 모양으로 돌아갑니다. 실제로 check.html 이 2026-08-19 에
 * 「쓰는 자리가 없어졌다」며 이 규칙을 걷은 전례가 있습니다(그때는 로그인 영역이 그
 * 페이지에 없어서 무해했습니다).
 */
test('로그인 영역을 싣는 페이지는 .nav-quiet 규칙을 갖는다 — auth.js 가 그 클래스로 그린다', () => {
  ...
  for (const f of pages) {
    ...
    assert.ok(/\.nav-quiet\s*\{[^}]*font-size:\s*14px/.test(css),
      f + ' 에 `.nav-quiet` 규칙이 없습니다 — 로그인 영역의 [로그인] 버튼이 색·크기를 잃습니다');
    assert.ok(/\.nav-quiet:hover\s*\{/.test(css),
      f + ' 에 `.nav-quiet:hover` 가 없습니다 — [로그인] 버튼이 hover 를 잃습니다');
  }
});
