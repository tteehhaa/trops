/**
 * 랜딩 언어 분기 — 기본은 영어, `lang=ko` 쿠키가 있을 때만 국문을 그대로 보여준다.
 * 〔2026-08-21 · 창업자 지시 — 랜딩 영어 우선 접속 + 비영어권 방문자 언어 선택〕
 *
 * 쿠키는 `lang`(ko|en) 값 하나뿐이고 기능성(언어 선택 기억) 목적이다 — 광고·분석이
 * 아니다(privacy.html §01 예외 참조, 2026-08-21). 검색엔진 크롤러도 예외 없이
 * 리다이렉트된다(쿠키가 없으므로) — 창업자 결정이고, 국문 홈의 검색 노출이 흔들릴 수
 * 있다는 점을 알고 받아들인 것이다.
 *
...
export default function middleware(request) {
  ...
  const chosenKo = /(?:^|;\s*)lang=ko(?:;|$)/.test(cookieHeader);
  if (chosenKo) return; // 이미 한국어를 골랐다 — 국문 그대로 서빙
