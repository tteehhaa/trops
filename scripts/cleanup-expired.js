 ...
 *   삭제는 service_role 키가 필요합니다. 그 키를 쥔 경로를 공개 주소에 두면
 *   토큰 하나가 새는 순간 전체 삭제가 가능해집니다. 키 회전이 끝나기 전까지는
 *   공개 표면을 늘리지 않고 손으로(또는 사설 러너에서) 돌립니다.
 *   Vercel Cron 으로 옮기는 판단은 키 회전 후에 다시 합니다.
 *
 * ── 실행 ────────────────────────────────────────────────────────────────────
...
const { readConfig, safeText, storageKey, removeObjects, listObjects } =
...
async function main(argv) {
  ...
  try {
    await cleanupExpired(config, options);
    if (orphans) await cleanupOrphans(config, options);
  } catch (err) {
