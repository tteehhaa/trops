...
const STATIC = {
   ...
   *       띄웠다 — 죽은 앱 화면보다 살아 있는 우리 페이지가 낫다. 토큰이 가리키던 결과지는
   *       어디로 보내도 이제 없다(그 자료도 2026-09-09 에 전부 삭제됐다).
   *       🔄 **2026-09-21 — 그 200 도 끝났다.** `/precheck` 가 앱 `quick-check` 로 영구
   *          이동하므로 그 링크는 **토큰이 실린 채 앱 1분 진단 화면**에 도착한다 —
   *          쿼리는 그대로 넘어가고 앱에 그 토큰을 쓸 자리가 없다(아래 등재 참조).
  ...
  html: [
    ...
    /*
     * ⛔ **`precheck.html` 을 내렸습니다 — 배포에서만입니다** 〔2026-09-21 · 대표 결정 · 배치 6〕.
     *
     * 1분 진단의 자리가 `app.trops.kr/quick-check` 로 옮겼습니다(L1). 랜딩 버튼 여섯이
     * 전부 그리로 가고, 이 페이지의 응답 전송도 껐습니다(L2). 즉 **진입원이 0** 이고
     * 남겨 두면 같은 제품이 두 주소에 서 있게 됩니다.
     * 🔴 **`vercel.json` 이 `/precheck` 를 `permanent: true` 로 앱에 넘깁니다** — 그 파일의
     *    다른 네 규칙이 `false` 인 것과 다릅니다. 여기서는 **색인을 옮기는 것이 목적**이라
     *    (같은 배치에서 `llms.txt`·JSON-LD 두 장의 주소도 함께 옮겼습니다) 영구가 맞습니다.
     *    ⚠️ 영구는 브라우저가 캐시합니다 — 되돌리려면 먼저 `permanent` 를 `false` 로 바꾼
     *       판을 한 번 배포해야 합니다.
     *
     * 🔴 **⛔ 파일은 «지우지 않았습니다» — 위 2026-08-30 의 「파일도 함께 지웠다」와 다릅니다.**
     *    앱 저장소의 검사 다섯이 이 파일을 **읽어** 배점표를 맞댑니다(대표적으로
     *    `tests/quick-check/score-cross-repo.test.ts` — 앱 `lib/quick-check/score.ts` 는 이
     *    파일의 `<script>` 에서 이식한 사본이고, 두 판이 갈리면 그 검사가 red 를 냅니다).
     *    지우면 그 대조가 **조용히 꺼지고**(그 파일이 `existsSync` 로 건너뜁니다) 앱의 점수가
     *    랜딩 원본과 갈려도 아무도 모릅니다.
     *    ⛔ 「배포 안 하는 파일이 남아 있다」를 조용한 실패로 읽지 마십시오 — **이 주석이
     *       그 사유이고**, 이 저장소에서 유일한 예외입니다.
     *    🔄 지워도 되는 날 = 앱이 랜딩 원본과의 대조를 그만두는 날입니다(그때는 앱의
     *       고정 케이스표 `tests/fixtures/quick-check/landing-cases.json` 가 유일한 회귀선이
     *       되므로, ⛔ 그 케이스표까지 함께 지우지 마십시오).
     *
     * ⚠️ 이 파일을 읽는 «이 저장소»의 검사도 그대로 삽니다(`quick-check-report` ·
     *    `precheck-handoff` · `i18n-parity` · `disclaimer-parity` · `price-exposure`).
     *    ⛔ 그 검사들이 도는 것을 「페이지가 배포된다」로 읽지 마십시오.
     */
    /*
     ...
     *    `/about` 을 가리킵니다. nav 에는 넣지 않았습니다: 헤더는 「1분 수출 준비 진단」
   ...
   */
  /*
   * 🔴 **네이버 소유확인 파일** 〔2026-09-09〕 — 이름과 위치가 그대로 규약이라 이 칸입니다.
   *    내용은 한 줄(67바이트)이고 손대면 안 됩니다. 서치어드바이저가 내려 준 원본입니다.
   *
   * ⚠️ **이 주소는 «308 을 한 번 거쳐» 열립니다.** `vercel.json` 의 `cleanUrls: true` 가
   *    `.html` 로 끝나는 요청을 확장자 없는 주소로 무조건 돌리기 때문입니다(파일이
   *    있든 없든 돕니다 — 2026-09-09 실측). 그래서 실제 경로는
   *      /navera3294….html → 308 → /navera3294… → 200 (이 파일)
   *    입니다. 네이버가 그 308 을 따라가야 확인이 납니다.
   * ⛔ 미들웨어로 200 을 직접 내주는 방법은 **되지 않습니다.** Vercel 이 cleanUrls
   *    리다이렉트를 미들웨어보다 먼저 처리합니다(커밋 c386b76 에서 배포해 확인했고
   *    되돌렸습니다). 같은 시도를 다시 하지 마십시오.
   * 🔴 소유확인이 끝나면 이 줄과 파일을 함께 지우십시오 — 남겨 두면 토큰이 계속 공개됩니다.
   */
  files: [
    'favicon.ico',
    'robots.txt',
    'llms.txt',
    'navera3294de2d83b96c80b628307d539f51a.html',
  ],
};
...
const NOT_DEPLOYED = new Set([
  /*
   * 🔄 **`precheck.html` 이 여기서 내려갔습니다** 〔2026-09-21 · 대표 결정 「지우자」〕.
   *    같은 날 오전에 이 자리에 등재하며 적어 둔 사유는 「앱 저장소의 배점표 대조가 이
   *    파일을 읽으므로 지우면 그 대조가 `existsSync` 에서 조용히 꺼진다」였습니다.
   *    오후에 **앱이 그 원본을 자기 안으로 동결해 들이면서**(`trops_a@11d3699f` ·
   *    `tests/fixtures/quick-check/landing-score-source.js`) 그 사유가 사라졌고,
   *    형제 저장소 인계 문서가 적어 둔 조건 「지워도 되는 날 = 앱이 랜딩 원본과의 대조를
   *    그만두는 날」이 충족됐습니다. 파일도 함께 지웠습니다.
   * ⛔ 여기에 다시 등재하려거든 **페이지를 되살릴 것인지부터** 정하십시오 —
   *    `test/quick-check-report.test.js` 머리주석이 그 순서를 갖습니다.
   */
  /*
   * 서버리스 함수 — Vercel 이 별도로 처리합니다(이 스크립트가 dist/ 로 옮기지 않습니다).
   *
   * 🔴 **지금 이 폴더는 없습니다** 〔2026-09-20〕 — 마지막 함수(`api/prestep.js`)를 걷었고,
   *    그래서 trops.kr 에 서버 엔드포인트가 **0개**입니다. 랜딩은 앱 엔드포인트
   *    (`app.trops.kr/api/*`)를 브라우저에서 직접 부릅니다.
   * ⛔ 여기에 파일을 두면 **그 순간 공개 엔드포인트가 됩니다** — 아무나 부를 수 있고,
   *    함수가 읽는 env 는 프로덕션에 비밀값을 하나 더 세운다는 뜻입니다.
   *    `test/no-secrets.test.js` 가 그 사실을 red 로 알립니다. 새로 둘 이유가 생기면
   *    그 검사를 고치면서 **왜 필요한지를 그 자리에 적으십시오.**
   * ⚠️ 이름은 남겨 둡니다 — 폴더가 다시 생겼을 때 「분류표에 없는 항목」으로 빌드가
   *    멈추는 것보다, 검사가 이름을 대고 막는 편이 읽힙니다.
   */
  'api',
