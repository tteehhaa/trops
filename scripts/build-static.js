...
const NOT_DEPLOYED = new Set([
  /*
   * 🔴 **배포는 안 하지만 «앱 검사가 읽는» 파일** 〔2026-09-21 · 배치 6〕.
   *    1분 진단의 자리가 앱으로 옮겨 `/precheck` 는 `vercel.json` 이 앱으로 넘깁니다.
   *    그런데 앱 저장소의 배점표 대조(`tests/quick-check/score-cross-repo.test.ts` 외)가
   *    이 파일을 **읽어** 앱 `lib/quick-check/score.ts` 와 맞댑니다 — 지우면 그 대조가
   *    `existsSync` 에서 **조용히 꺼집니다**. 사유는 STATIC.html 의 같은 자리 주석이 갖습니다.
   *    ⛔ 미추적 임시 사본과 혼동하지 마십시오(위 머리말) — 이것은 추적되는 정본이고,
   *       배포만 하지 않습니다.
   */
  'precheck.html',
  /*
