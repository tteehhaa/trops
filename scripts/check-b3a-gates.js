 ...
 *
 * ✅ **그 남은 물음이 닫혔습니다** 〔2026-09-21 · 대표 결정 「1분 수출 준비 진단으로 통일 ·
 *    랜딩을 맞춘다」〕. 종전에는 **한 목적지에 이름이 셋**이었습니다 — nav·히어로의
 *    「1분 무료 진단」, 1단계 카드 제목 「수출 사전점검 리포트」, 그 버튼 「사전점검
 *    시작하기」가 **전부 `app.trops.kr/quick-check`** 로 갔고, 도착한 화면의 제목은
 *    **「1분 수출 준비 진단」**이었습니다. 앱 제목으로 모았습니다(JSON-LD `name` 포함).
 *
 * 🔴 **그래서 아래 ⛔ 는 «이제 해소된» 금지입니다** — 종전 문장은 「어느 쪽으로 모을지
 *    정해지지 않았으니 한쪽을 임의로 고치지 말 것」이었고, 그 조건이 충족됐습니다.
 *    ⛔ 그렇다고 **되돌리지는 마십시오** — 되돌리려면 그 결정을 다시 받아야 합니다.
 * ⚠️ **1단계의 «산출물 축» 이름이 사라졌습니다** — 2026-09-03 개편은 활동 축(앱 탭)과
 *    산출물 축(파는 것)을 갈라 1단계를 「…리포트」로 불렀습니다. 지금 그 카드는 활동
 *    이름을 씁니다. 2·3단계(「무역보험 준비 패키지」·「수출대금 관리」)는 그대로입니다.
 * ⚠️ **FAQ 본문은 아직 「수출 사전점검」으로 부릅니다** — 그 이름의 실제 제품이
 *    `app.trops.kr/export-precheck`(14항목·로그인)로 따로 있고, 랜딩이 그쪽을 가리키는
 *    링크는 0건입니다. **랜딩이 그 제품을 다룰 것인지**가 아직 미결이라 손대지 않았습니다.
 *
